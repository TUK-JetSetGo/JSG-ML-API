...
class EnhancedTabuSearchModel:
    def _get_spot_id(self, spot: TouristSpot) -> int:
        ...
        return getattr(spot, "tourist_spot_id", getattr(spot, "id", -1))  # Return -1 or raise error if no ID
    ...
    def _get_transport_speed(self, user_profile: Optional[UserProfile]) -> float:
        if user_profile and user_profile.preferred_transport:
            return DEFAULT_TRANSPORT_SPEEDS.get(user_profile.preferred_transport.lower(),
        ...
        for i in range(n):
            for j in range(n):
                ...
                if spots[i].coordinate and spots[j].coordinate:
                    distance_matrix[i][j] = spots[i].coordinate.distance_to(spots[j].coordinate)
                else:
                    logger.warning(
    ...
    def _calculate_route_properties(
        ...
        total_travel_time = total_distance / transport_speed_kmh if transport_speed_kmh > 0 else float("inf")
        ...
        for k, spot_idx in enumerate(visited_spots_in_order_indices):
            order = k + 1
            if 0 <= spot_idx < len(spots):
                current_score += priority_scores.get(order, {}).get(self._get_spot_id(spots[spot_idx]), 0.0)
            else:
                logger.error(f"Invalid spot_idx in priority score calculation: {spot_idx}")
        ...
        if prev_day_end and visited_spots_in_order_indices:
            ...
            if 0 <= first_spot_idx < len(spots):
                ...
                if prev_day_end.coordinate and first_spot_obj.coordinate:
                    dist_to_first = prev_day_end.coordinate.distance_to(first_spot_obj.coordinate)
                    current_score += continuity_weight * (100.0 * math.exp(-dist_to_first / 10.0))
                else:
    ...
    def _is_route_feasible(
        ...
        if not route_indices or route_indices[0] != start_spot_index or route_indices[-1] != start_spot_index:
            return False
    ...
    def _generate_initial_solution(
        ...
        if must_visits_to_insert:
            for _ in range(len(must_visits_to_insert)):
                ...
                remaining_must_visits_to_try = [mv_idx for mv_idx in must_visits_to_insert if
                                                mv_idx not in current_inserted_spots_indices]
                ...
                for must_visit_idx_to_try in remaining_must_visits_to_try:
                    for i in range(len(current_route) - 1):
                        temp_route = current_route[:i + 1] + [must_visit_idx_to_try] + current_route[i + 1:]
                        temp_must_visit_set_check = current_inserted_spots_indices.union({must_visit_idx_to_try})
                        obj_val, dist, dur = self._calculate_route_properties(temp_route, spots, distance_matrix,
                                                                              base_scores, priority_scores,
                                                                              transport_speed_kmh, prev_day_end,
                                                                              continuity_weight)
                        if self._is_route_feasible(temp_route, dist, dur, max_distance, max_duration, max_places,
                                                   temp_must_visit_set_check, start_spot_index):
        ...
        final_obj_mv, final_dist_mv, final_dur_mv = self._calculate_route_properties(current_route, spots,
                                                                                     distance_matrix, base_scores,
                                                                                     priority_scores,
                                                                                     transport_speed_kmh, prev_day_end,
                                                                                     continuity_weight)
        if not self._is_route_feasible(current_route, final_dist_mv, final_dur_mv, max_distance, max_duration,
                                       max_places, must_visit_spot_indices_set, start_spot_index):
            logger.warning(
                f"초기 해 생성: 필수 방문지 삽입 후 경로 제약 조건 위반. 필수 인덱스: {must_visit_spot_indices_set}, 경로: {current_route}")
            return [start_spot_index, start_spot_index]

        optional_indices_to_try = list(set(range(len(spots))) - {start_spot_index} - must_visit_spot_indices_set)
        ...
        for _ in range(len(optional_indices_to_try)):
            ...
            current_route_obj_val = \
                self._calculate_route_properties(current_route, spots, distance_matrix, base_scores, priority_scores,
                                                 transport_speed_kmh, prev_day_end, continuity_weight)[0]
            ...
            remaining_optional_to_try = [opt_idx for opt_idx in optional_indices_to_try if
                                         opt_idx not in current_spots_in_route_indices]
            ...
            for optional_idx_to_try in remaining_optional_to_try:
                if len(set(current_route[1:-1] + [optional_idx_to_try])) > max_places:
                    continue
                for i in range(len(current_route) - 1):
                    temp_route = current_route[:i + 1] + [optional_idx_to_try] + current_route[i + 1:]
                    obj_val, dist, dur = self._calculate_route_properties(temp_route, spots, distance_matrix,
                                                                          base_scores, priority_scores,
                                                                          transport_speed_kmh, prev_day_end,
                                                                          continuity_weight)
                    if self._is_route_feasible(temp_route, dist, dur, max_distance, max_duration, max_places,
                                               must_visit_spot_indices_set, start_spot_index):
    ...
    def _tabu_search_core(
            ...
            h_min: int = 10, h_max: int = 20,
            t_min: int = 3, t_max: int = 7,
    ) -> List[int]:
        logger.info(f"TS Core: 초기 경로: {initial_route}, 필수 방문지 인덱스: {must_visit_spot_indices_set}")
        best_solution_route = list(initial_route)
        current_solution_route = list(initial_route)
        best_obj_val, best_dist, best_dur = self._calculate_route_properties(best_solution_route, spots,
                                                                             distance_matrix, base_scores,
                                                                             priority_scores, transport_speed_kmh,
                                                                             prev_day_end, continuity_weight)

        logger.info(f"TS Core: 초기 최적 목적 함수 값: {best_obj_val:.2f}, 거리: {best_dist:.2f}, 시간: {best_dur:.2f}")

        if best_obj_val == -float("inf") or not self._is_route_feasible(best_solution_route, best_dist, best_dur,
                                                                        max_distance, max_duration, max_places,
                                                                        must_visit_spot_indices_set, start_spot_index):
            logger.warning("TS Core: 초기 해가 제약 조건을 만족하지 못하여 Tabu Search를 진행할 수 없습니다.")
        ...
        for iter_count in range(max_iterations):
            ...
            if len(temp_current_route) > 3:  # Need at least two spots to swap besides start/end
                # Attempt SWAP move
                # Ensure we are swapping actual visited spots, not start/end points if they are part of the list of visited spots
                visitable_indices_in_route = [idx for idx in range(1, len(temp_current_route) - 1) if
                                              temp_current_route[idx] != start_spot_index]
                if len(visitable_indices_in_route) >= 2:
                    idx1_route_pos, idx2_route_pos = random.sample(visitable_indices_in_route, 2)
                    ...
                    swapped_route[idx1_route_pos], swapped_route[idx2_route_pos] = node_idx_at_pos2, node_idx_at_pos1
                    move_made_type = "swap"
                    move_attribute = frozenset({node_idx_at_pos1, node_idx_at_pos2})

                    neigh_obj, neigh_dist, neigh_dur = self._calculate_route_properties(swapped_route, spots,
                                                                                        distance_matrix, base_scores,
                                                                                        priority_scores,
                                                                                        transport_speed_kmh,
                                                                                        prev_day_end,
                                                                                        continuity_weight)
                    is_feasible = self._is_route_feasible(swapped_route, neigh_dist, neigh_dur, max_distance,
                                                          max_duration,
                                                          max_places, must_visit_spot_indices_set, start_spot_index)

                    if is_feasible:
                        is_tabu = tabu_list_moves.get((move_made_type, move_attribute), 0) > iter_count
                        ...
                        if (not is_tabu) or aspiration_met:
                            if neigh_obj > best_neighbor_obj_val_candidate:
                                ...
                                best_move_details_candidate = (move_made_type, move_attribute)
            ...
            if best_neighbor_route_candidate is not None:
                ...
                if current_obj_val > best_obj_val:
                    ...
                    _, best_dist, best_dur = self._calculate_route_properties(best_solution_route, spots,
                                                                              distance_matrix, base_scores,
                                                                              priority_scores, transport_speed_kmh,
                                                                              prev_day_end, continuity_weight)
                    logger.debug(
            ...
            if iter_count % 50 == 0:  # Periodically clean
                tabu_list_moves = {move: expiry for move, expiry in tabu_list_moves.items() if expiry > iter_count}
    ...
    def optimize_route(
            ...
            base_scores: Dict[int, float],
            priority_scores: Dict[int, Dict[int, float]],
            must_visit_spot_ids_input: Optional[Union[Set[int], List[Set[int]]]] = None,  # Added
        ...
        """
        is_multi_day = isinstance(daily_spots_input, list) and \
                       bool(daily_spots_input) and \
                       isinstance(daily_spots_input[0], list)
        ...
        daily_spots_list: List[List[TouristSpot]] = daily_spots_input if is_multi_day else [
            daily_spots_input]  # type: ignore
        daily_start_indices_list: List[int] = daily_start_indices_input if is_multi_day else [
            daily_start_indices_input]  # type: ignore
        daily_max_distance_list: List[float] = daily_max_distance_input if is_multi_day else [
            daily_max_distance_input]  # type: ignore
        daily_max_duration_list: List[float] = daily_max_duration_input if is_multi_day else [
            daily_max_duration_input]  # type: ignore
        ...
        if must_visit_spot_ids_input is None:
            ...
            daily_must_visit_spot_ids_list = [must_visit_spot_ids_input] if not is_multi_day else [
                                                                                                      must_visit_spot_ids_input] + [
                                                                                                      None] * (
                                                                                                              len(daily_spots_list) - 1)  # Apply to first day if multi-day and single set given
            ...
            if len(must_visit_spot_ids_input) == len(daily_spots_list):
                ...
                logger.error("Length mismatch: must_visit_spot_ids_input list and daily_spots_list")
                return []  # Or raise error
        else:
            logger.error(f"Invalid type for must_visit_spot_ids_input: {type(must_visit_spot_ids_input)}")
            return []
        ...
        for day_idx, current_day_spots in enumerate(daily_spots_list):
            ...
            must_visit_ids_for_day: Optional[Set[int]] = daily_must_visit_spot_ids_list[day_idx]
            ...
            spot_id_to_idx_map = {self._get_spot_id(spot): i for i, spot in enumerate(current_day_spots)}
            if must_visit_ids_for_day:
                for spot_id in must_visit_ids_for_day:
                    if spot_id in spot_id_to_idx_map:
                        must_visit_spot_indices_set_for_day.add(spot_id_to_idx_map[spot_id])
                    else:
                        logger.warning(
            ...
            if not initial_route_indices or len(initial_route_indices) < 2:  # Check if initial solution is valid
                logger.warning(
                ...
                if initial_route_indices == [start_spot_index_for_day, start_spot_index_for_day]:
            ...
            final_score, final_dist, final_dur = self._calculate_route_properties(
                optimized_route_indices_for_day, current_day_spots, distance_matrix_for_day,
                base_scores, priority_scores, transport_speed_kmh, prev_day_end_spot, continuity_weight
            )

            # Validate final route from Tabu Search
            if not self._is_route_feasible(optimized_route_indices_for_day, final_dist, final_dur, max_dist_for_day,
                                           max_dur_for_day, max_places_per_day, must_visit_spot_indices_set_for_day,
                ...
                initial_score, initial_dist, initial_dur = self._calculate_route_properties(initial_route_indices,
                                                                                            current_day_spots,
                                                                                            ...
                                                                                            prev_day_end_spot,
                                                                                            continuity_weight)
                if self._is_route_feasible(initial_route_indices, initial_dist, initial_dur, max_dist_for_day,
                                           max_dur_for_day, max_places_per_day, must_visit_spot_indices_set_for_day,
                                           start_spot_index_for_day):
                    logger.info(f"Day {day_idx + 1}: Falling back to initial feasible solution.")
                    all_optimized_routes_info.append((initial_route_indices, initial_dist, initial_dur))
                    if initial_route_indices and len(initial_route_indices) > 1 and initial_route_indices[-2] < len(
                            current_day_spots):
                        prev_day_end_spot = current_day_spots[
                            initial_route_indices[-2]]  # Second to last is the actual last visited spot
                    else:
                        prev_day_end_spot = None
                else:
                    logger.info(f"Day {day_idx + 1}: No feasible solution found, returning empty route for the day.")
                    all_optimized_routes_info.append(([], 0.0, 0.0))
                    prev_day_end_spot = None
            else:
                all_optimized_routes_info.append((optimized_route_indices_for_day, final_dist, final_dur))
                if optimized_route_indices_for_day and len(optimized_route_indices_for_day) > 1 and \
                        optimized_route_indices_for_day[-2] < len(current_day_spots):
                    prev_day_end_spot = current_day_spots[
                        optimized_route_indices_for_day[-2]]  # Second to last is the actual last visited spot
                else:
                    prev_day_end_spot = None
