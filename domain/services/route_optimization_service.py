...
from infrastructure.adapters.ml.enhanced_tabu_search_model import EnhancedTabuSearchModel
...
class RouteOptimizationService:
    ...
    def generate_itinerary(
        ...
        """
        is_multi_day_request = isinstance(daily_spots_input, list) and \
        ...
        if not daily_spots_input:
            logger.warning("입력된 일일 관광지 정보(daily_spots_input)가 비어있습니다. 빈 결과를 반환합니다.")
            return []

        if is_multi_day_request:
            num_days = len(daily_spots_input)
            if not (isinstance(daily_start_indices_input, list) and len(daily_start_indices_input) == num_days):
                logger.error(
                    f"데이터 불일치: {num_days}일 일정의 경우, `daily_start_indices_input`은 길이가 {num_days}인 리스트여야 합니다. 입력: {daily_start_indices_input}"
                )
                return []
            if must_visit_spot_ids_input is not None and not (isinstance(must_visit_spot_ids_input, list) and len(must_visit_spot_ids_input) == num_days):
                logger.error(
            ...
            if not isinstance(daily_start_indices_input, int):
                ...
                return []
            if must_visit_spot_ids_input is not None and not isinstance(must_visit_spot_ids_input, set):
                logger.error(
    ...
    def optimize_route(
        ...
        if not spots:
            logger.warning("optimize_route: 입력된 관광지(spots)가 비어있습니다. 빈 결과를 반환합니다.")
            return [], 0.0, 0.0
        ...
        if spots and must_visit_indices:
            for idx in must_visit_indices:
                if 0 <= idx < len(spots):
                    spot_id = getattr(spots[idx], "tourist_spot_id", getattr(spots[idx], "id", None))
                    if spot_id is not None:
                        must_visit_spot_ids.add(spot_id)
                    else:
                        logger.warning(f"관광지 ID를 찾을 수 없습니다: spots[{idx}] (must_visit_indices)")
                else:
        ...
        if results and isinstance(results, list) and len(results) > 0 and isinstance(results[0], tuple) and len(results[0]) == 3:
            return results[0]
        else:
            logger.warning("optimize_route: generate_itinerary로부터 유효한 결과를 받지 못했습니다.")
            return [], 0.0, 0.0
